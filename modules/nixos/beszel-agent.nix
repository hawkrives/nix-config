...
# universal token, so there is no listening port and nothing to open in the
# firewall. HUB_URL is nutmeg's tailscale IP rather than a name because it has
...
{ config, ... }:
{
  ...
  services.beszel.agent = {
    ...
    environment = {
      ...
      # keypair means updating this value.
